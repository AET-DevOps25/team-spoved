...
def fetch_data_video_photo(media_id: int):
    response = requests.get(f"{API_URL}/video-photo/{media_id}")
    return response.json()

# Update analyzed status
def update_analyzed(media_id: int, analyzed: bool):
    response = requests.put(f"{API_URL}/video-photo/{media_id}/analyzed", json={"analyzed": analyzed})
    return response.json()

# Update result
def update_result(media_id: int, result: str):
    response = requests.put(f"{API_URL}/video-photo/{media_id}/result", json={"result": result})
    return response.json()

# Update reason
def update_reason(media_id: int, reason: str):  
    response = requests.put(f"{API_URL}/video-photo/{media_id}/reason", json={"reason": reason})
    return response.json()
...
async def update_analyzed_status(media_id: int, data: dict):
    """Update analyzed status"""
    return update_analyzed(media_id, data.get("analyzed", False))
